const bcrypt = require('bcrypt');
const { response } = require('express');
const saltRounds = 10;

const { pool } = require('../config')
const { transporter } = require('../libs/mailer');


//seg_usuarios
const getUsuarios = async (req, res) => {
    try {
        const response = await pool.query(`SELECT * from usuarios order by apellido asc`);
        res.status(200).json(response.rows);
    } catch (error) {
        res.status(500).json(error);
    }
};


//get usuario
const getUsuario = async (req, res) => {
    const { id_usuario } = req.params;
    try {
        const response = await pool.query('SELECT * FROM usuarios WHERE id_usuario = $1 ', [id_usuario]);

        if (response.rowCount > 0) {
            res.status(200).json(response.rows);
        } else {
            res.status(200).json(response.rows)
        }
    } catch (error) {
        res.status(500).json(error);
    }
};


//create usuarios
const createUsuario = async (req, res) => {
    try {
        //id_estado int
        const { dni, nombre, apellido, email, id_estado, telefono, id_sector, id_posicion, jefe_directo, dias_vacaciones, n_legajo,id_empresa } = req.body;
        var pssHash = '';
        var pss = dni;
        //console.log(n_legajo);
         // Convertir nombre y apellido a mayúsculas
         const nombreMayuscula = nombre.toUpperCase();
         const apellidoMayuscula = apellido.toUpperCase();
        //comprobar que el dni del usuario no exista
        const verify = await pool.query('SELECT * FROM usuarios WHERE dni = $1', [dni]);
        if (verify.rowCount > 0) {
            res.status(501).json({
                message: 'El usuario ya existe'
            });
        } else {
            bcrypt.genSalt(saltRounds, function (err, salt) {
                bcrypt.hash(pss, salt, async function (err, hash) {
                    pssHash = hash;
                    const response = await pool.query('INSERT INTO usuarios (dni,nombre,apellido,password,email,id_estado,telefono) VALUES ($1, $2, $3, $4, $5, $6, $7)', [dni, nombreMayuscula, apellidoMayuscula, pssHash, email, id_estado, telefono]);
                    const user = await pool.query(`select * from usuarios where dni=$1`, [dni])
                    //enviar correo electronico
                    //Cargar empleado
                    const id_usuario = user.rows[0].id_usuario;
                    const empleado = await pool.query(`insert into empleados(id_empleado,fecha_alta, id_sector,id_posicion,jefe_directo,dias_vacaciones,dias_restantes,n_legajo,id_empresa)values($1,now(),$2,$3,$4,$5,$6,$7,$8);`, [id_usuario, id_sector, id_posicion, jefe_directo, dias_vacaciones, dias_vacaciones, n_legajo,id_empresa]);
                    res.status(200).json({
                        message: 'Usuario creado'
                    });
                });
            });
        }
    } catch (error) {
        res.status(500).json({
            message: 'Error al crear usuario'
        });
    }
};


//edit usuario nombre, apellido, email, telefono, dni
const editUsuario = async (req, res) => {
    try {
        const { nombre, apellido, email, telefono,dni } = req.body;
        const response = await pool.query('UPDATE usuarios SET nombre = $1, apellido = $2, email = $3, telefono = $4 WHERE dni = $5', [nombre, apellido, email, telefono, dni]);
        if (response.rowCount > 0) {
            res.status(200).json({
                message: 'Usuario editado'
            });
        } else {
            res.status(501).json({
                message: 'Usuario no encontrado'
            });
        }

    } catch (error) {
        res.status(500).json({
            message: 'Error al editar usuario'
        });
    }
};


// Alta User
//1= activo
//2 =inactivo
const altaUser = async (req, res) => {
    const { dni } = req.body;
    try {
        //Verificar q exista el usuario
        const user = await pool.query('select * from usuarios where dni=$1', [dni]);
        const email = user.rows[0].email;
        if (user.rowCount > 0) {
            const estado_usuario = 2;
            //Verificar que el estado del usuario sea 0, este de baja
            const verify = await pool.query('SELECT * FROM usuarios WHERE dni = $1 and id_estado = $2', [dni, estado_usuario]);
            // const dni=verify.rows[0].email;
            //console.log(verify.rows[0].id_usuario)
            const nombre = verify.rows[0].nombre;
            const apellido = verify.rows[0].apellido;
            const email = verify.rows[0].email;
            if (verify.rowCount > 0) {
                const estado = 1;
                const response = await pool.query('UPDATE usuarios SET id_estado = $1 WHERE dni = $2', [estado, dni]);
                //Enviar mail de notificacion
                // send mail with defined transport object
                /*                 let info = await transporter.sendMail({
                                    from: '"Alta User" <notificaciones@acme.com.ar>', // sender address
                                    to: `${email}`, // list of receivers
                                    subject: "Alta Usuario Acme ✔", // Subject line
                                    //text: "Hello world?", // plain text body
                                    html: `
                                    <div style="margin: auto;width: 75%;">
                        <div
                            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
                            <div>
                                <div style="background-color: #333333; height: 22px;">
                
                                </div>
                                <div style="background-color: #FFFFFF; height:25px">
                                    <h4>Alta User</h4>
                                </div>
                                <div style="background-color: #FFCC00 ;height:180px;">
                                    <a href="https://ibb.co/ZBbvhLt"><img
                                            src="data:image/png;base64,"
                                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>
                
                                </div>
                                <div style="background-color: #333333;height: 10px;"></div>
                            </div>
                
                
                            <div style="padding-top: 2em; ">
                                <div>
                
                                    <div style="text-align: left;padding-left: 10%;">
                                        <p style="font-family: serif;">Hola </p>
                                        <h4>${nombre} ${apellido}</h4>
                                        <p style="font-family: serif;"> Te damos la bienvenida a Acme.
                                            <br>
                                            Una vez que ingreses al sistema te va a pedir que modifiques la contraseña por única vez.
                
                                        </p>
                                    </div>
                                </div>
                                
                                    
                                        <div>
                                            <h5>Link:</h5>
                                        </div>
                                    
                                    
                                        <div style="background-color: #E8E8E8;height: 45px; padding-top: 1.5em;margin: 0 10% 0 10%">
                                            <a href="http://app.acme.com.ar/">app.acme.com.ar</a>
                                        </div>
                                    
                                    
                                        <div>
                                            <h5>Usuario:</h5>
                                        </div>
                                   
                                    
                                        <div style="background-color: #E8E8E8;height: 65px;margin: auto; padding-top: 1px;margin: 0 10% 0 10% ">
                                            <h5>N° de dni sin puntos ni separacion (xxxxxxxx)</h5>
                                        </div>
                                    
                                        <div>
                                            <h5>Contraseña:</h5>
                                        </div>
                                    
                                        <div style="background-color: #E8E8E8;height: 65px;margin: auto;padding: 1px;margin: 0 10% 0 10%;">
                                            <h5>N° de dni sin puntos ni separacion (xxxxxxxx)</h5>
                                        </div>
                                    
                                
                
                
                                <div style="padding: 2em 0 1em; margin: 0 10% 0 10% ;">
                                    <p style="font-family: serif;text-align: left;">Si perdiste tu contraseña, deberas comunicarte a RRHH para
                                        solicitar el reinicio de la misma.
                                    </p>
                                </div>
                            </div>
                        </div>
                
                        <div>
                            <div style="background-color: #FFCC00; height: 7px;"></div>
                            <div style="background-color: #E8E8E8;height: 60px; padding-top: 1px;">
                                <div style="margin: auto;text-align: center;">
                                    <h5><a href="http://app.acme.com.ar/" style="margin: auto;">app.acme.com.ar</a></h5>
                                </div>
                            </div>
                
                
                
                        </div>
                        <div style="padding: 4em,0em,4em; background-color: #F4F4F3;height: 200px; margin: auto;">
                            <div style="padding-top: 1em;font-family: serif;margin: 0 10% 0 10%">
                                <div>
                                    <p>Por cualquier consulta comunicate a nuestros telefonos o mail.</p>
                                </div>
                                <div style="font-family: serif;">
                                    <h4> Teléfono: (297) 5003371</h4>
                                    <h4> recursoshumanos@acme.com.ar</h4>
                                </div>
                            </div>
                        </div>
                        <div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
                    </div>
                            
                                    `, // html body
                                });
                
                 */
                res.status(200).json({ message: 'Usuario dado de alta correctamente' });

            } else {
                res.status(501).json({ message: 'El usuario ya está dado de alta' })
            }
        } else {
            res.status(501).json({ message: 'Usuario no encontrado' })
        }
    }
    catch (error) {
        res.status(500).json({ message: 'No se puedo actualizar estado' });
    }
}


//baja usuario
const bajaUser = async (req, res) => {
    const { dni } = req.body;
    try {
        //Verificar q exista el usuario
        const user = await pool.query('select * from usuarios where dni=$1', [dni]);
        if (user.rowCount > 0) {
            const estado_usuario = 1;
            //Verificar que elestado del usuario sea 2
            const verify = await pool.query('SELECT * FROM usuarios WHERE dni = $1 and id_estado = $2', [dni, estado_usuario]);
            if (verify.rowCount > 0) {
                const estado = 2;
                const response = await pool.query('UPDATE usuarios SET id_estado = $1 WHERE dni = $2', [estado, dni]);
                res.status(200).json({ message: 'Usuario dado de baja' });
            } else {
                res.status(501).json({ message: 'El usuario ya esta dado de baja' })
            }
        } else {
            res.status(501).json({ message: 'Usuario no encontrado' })
        }
    }
    catch (error) {
        res.status(500).json({ message: 'No se pudo dar de baja el usuario' });
    }
}


//reset password
const resetPassword = async (req, res) => {
    try {
        const { dni } = req.body;
        var pssHash = '';
        var pss = dni;
        //verificar q exista el usuario
        const base = 1;
        const user = await pool.query('select * from usuarios where dni=$1 and reset_password = $2', [dni, base]);
        const nombre = user.rows[0].nombre;
        const apellido = user.rows[0].apellido;
        const email = user.rows[0].email;
        //console.log(user);
        if (user.rowCount > 0) {

            bcrypt.genSalt(saltRounds, function (err, salt) {
                bcrypt.hash(pss, salt, async function (err, hash) {
                    const pssHash = hash;
                    const reset = 0;
                    const response = await pool.query('UPDATE usuarios SET password = $1,reset_password = $2 WHERE dni = $3', [pssHash, reset, dni]);
                    res.status(200).json({
                        message: 'Password de usuario reseteado'
                    });

                    //
                });

            });

            //Enviar mail de notificacion
            // send mail with defined transport object
            let info = await transporter.sendMail({
                from: '"Reset Password" <notificaciones@acme.com.ar>', // sender address
                to: `${email}`, // list of receivers
                subject: "Resert Password ✔", // Subject line
                //text: "Hello world?", // plain text body
                html: `
                <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Reset Password</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 3em;margin: 0 10% 0 10% ">
                <div>

                    <div style="text-align: left;">

                        <h4>Marisa Carcamo.</h4>
                        <p style="font-family: serif;">
                            Le informamos que recientemente ha solicitado un reseteo de contraseña para su cuenta en
                            nuestro sistema.
                            <br>
                            <br>
                            Con el fin de garantizar la seguridad de su cuenta, es importante que siga
                            los siguientes pasos para cambiar su contraseña.
                            <br>
                            <br>
                            Deberá ingresar a su cuenta utilizando su nombre de usuario y la nueva contraseña temporal
                            (en este caso, su número de identificación personal o DNI).
                            <br>
                            <br>
                            Una vez que haya iniciado sesión en su cuenta, será dirigido automáticamente a la página de
                            cambio de contraseña. En esta página, podrá crear una nueva contraseña personalizada que
                            cumpla con los requisitos de seguridad del sistema.
                            <br>
                            Le recomendamos que seleccione una contraseña segura que incluya letras, números
                            y símbolos para proteger su cuenta de posibles amenazas de seguridad.
                            <br>
                            <br>
                            <br>
                            Si tiene alguna pregunta o necesita ayuda adicional, no dude en ponerse en contacto con
                            nuestro equipo de soporte técnico.
                            <br>
                            <br>
                            Atentamente.
                            <br>
                            <br>
                            Equipo IT
                        </p>
                    </div>
                </div>
            </div>
        </div>

        <div>
            <div style="background-color: #FFCC00; height: 7px;"></div>
            <div style="background-color: #E8E8E8;height: 60px; padding-top: 1px;">
                <div style="margin: auto;text-align: center;">
                    <h5><a href="http://app.acme.com.ar/" style="margin: auto;">app.acme.com.ar</a></h5>
                </div>
            </div>



        </div>
        <div style="padding: 4em,0em,4em; background-color: #F4F4F3;height: 200px; margin: auto;">
            <div style="padding-top: 1em;font-family: serif;margin: 0 10% 0 10%">
                <div>
                    <p>Por cualquier consulta comunicate a nuestros telefonos o mail.</p>
                </div>
                <div style="font-family: serif;">
                    <h4> Teléfono: (297) 5003371</h4>
                    <h4> recursoshumanos@acme.com.ar</h4>
                </div>
            </div>
        </div>
        <div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
    </div>

        
            
                    `, // html body
            });

        } else {
            res.status(500).json({ message: 'No se puede resetear el password del usuario' })
        }

    } catch (error) {
        res.status(500).json({ message: 'Error, no se pudo resetear password' })

    }
};

//cambiar password de usuario
const changePass = async (req, res) => {
    const { pass, newPass, id_usuario } = req.body;
    var oldPass = pass;
    var pssHash = '';
    //console.log(pass, newPass, id_usuario);
    try {
        const user = await pool.query('select * from usuarios where id_usuario = $1', [id_usuario]);
        if (user.rowCount > 0) {
            pssHash = user.rows[0].password;
            //console.log(pssHash);
            bcrypt.compare(pass, pssHash, function (err, result) {
                //console.log(result);
                if (!result) {
                    res.status(501).json({ message: 'pass no coincide' })
                } else {
                    bcrypt.genSalt(saltRounds, function (err, salt) {
                        bcrypt.hash(newPass, salt, async function (err, hash) {
                            const response = await pool.query('UPDATE usuarios SET password = $1,reset_password=1 WHERE id_usuario = $2', [hash, id_usuario]);
                        });
                    });
                    res.status(200).json({ message: 'pass modificado con exito' })
                }
            })
        }
        else {
            res.status(501).json({ message: 'Usuario no encontrado' })
        }
    } catch (error) {
        res.status(500).json({ message: 'No se pudo cambiar password' })
    }
}




module.exports = {
    bajaUser,
    changePass,
    resetPassword,
    getUsuarios,
    createUsuario,
    altaUser,
    getUsuario,
    editUsuario
} 