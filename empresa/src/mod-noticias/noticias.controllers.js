const { pool } = require('../config');
const { v4: uuidv4 } = require('uuid');
const { transporter } = require('../libs/mailer');


//Get todas las noticias
const getNews = async (req, res) => {
    try {
        const response = await pool.query(`select n.id_noticia, n.titulo, n.subtitulo, n.cuerpo_noticia, 
        to_char(n.fecha_publicacion::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_publicacion",
        to_char(n.fecha_vencimiento::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_vencimiento",
        n.url, n.id_categoria, n.id_estado, c.categoria
        from noticias n, categoria_noticias c
        where n.id_categoria = c.id_categoria
        order by n.id_noticia desc`);
        var query = `
        select n.id_noticia, n.titulo, n.subtitulo, n.cuerpo_noticia, n.fecha_vencimiento, n.url, n.id_categoria, n.id_estado, c.nombre_categoria
        from noticias n, categorias c
        where n.id_categoria = c.id_categoria
        order by n.id_noticia desc
        `
        if (response.rowCount > 0) {
            res.status(200).json(response.rows)
        } else {
            res.status(200).json(response.rows)
        }
    } catch (error) {
        res.status(500).json({ message: error });
    }
};


//Get noticias activas en fecha y estado = 1
const getActiveNews = async (req, res) => {
    try {
        const response = await pool.query(`select n.id_noticia, n.titulo, n.subtitulo, n.cuerpo_noticia, n.fecha_vencimiento, n.url, n.id_categoria, n.id_estado, c.categoria
        from noticias n, categoria_noticias c
        where n.id_categoria = c.id_categoria and fecha_vencimiento > current_timestamp and not id_estado = 0
        order by n.id_noticia desc`);
        var query = `
        select n.id_noticia, n.titulo, n.subtitulo, n.cuerpo_noticia, n.fecha_vencimiento, n.url, n.id_categoria, n.id_estado, c.nombre_categoria
        from noticias n, categorias c
        where n.id_categoria = c.id_categoria and fecha_vencimiento > current_timestamp and not id_estado = 0
        order by n.id_noticia desc
        `
        if (response.rowCount > 0) {
            res.status(200).json(response.rows)
        } else {
            res.status(200).json(response.rows)
        }
    } catch (error) {
        res.status(500).json({ message: error });
    }
};


const getNewsById = async (req, res) => {
    try {
        const { id } = req.params;
        const response = await pool.query(`select n.id_noticia, n.titulo, n.subtitulo, n.cuerpo_noticia, to_char(n.fecha_vencimiento::timestamp with time zone, 'yyyy-MM-dd'::text) AS "fecha_vencimiento",to_char(n.fecha_publicacion::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_publicacion", n.url, n.id_categoria, n.id_estado, c.categoria
        from noticias n, categoria_noticias c
        where n.id_categoria = c.id_categoria and n.id_noticia = $1`, [id]);

        var query = `
        select n.id_noticia, n.titulo, n.subtitulo, n.cuerpo_noticia, n.to_char(n.fecha_vencimiento::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_vencimiento", n.url, n.id_categoria, n.id_estado, c.categoria
        from noticias n, categoria_noticias c
        where n.id_categoria = c.id_categoria and n.id_noticia = $1
        `
        if (response.rowCount > 0) {
            res.status(200).json(response.rows)
        } else {
            res.status(200).json(response.rows)
        }
    }
    catch (error) {
        res.status(500).json({
            message: 'Error al obtener noticia'
        });
    }
};



const createNews = async (req, res) => {
    const { titulo, subtitulo, cuerpo_noticia, fecha_vencimiento, id_categoria, id_estado } = req.body;
    const file = req.file.path;
    try {
        const response = await pool.query('insert into noticias(titulo,subtitulo,cuerpo_noticia,fecha_vencimiento,url,id_categoria,id_estado)values($1,$2,$3,$4,$5,$6,$7)', [titulo, subtitulo, cuerpo_noticia, fecha_vencimiento, file, id_categoria, id_estado]);
        if (response.rowCount > 0) {
            //enviar mail
            let info = await transporter.sendMail({
                from: '"Nueva Noticia Publicada " <notificaciones@acme.com.ar>', // sender address
                to: `todos@acme.com.ar`, // Enviar lista de distribucion
                subject: `Nueva Noticia Publicada ✔`, // Subject line
                html: `
                    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">
                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Nueva Noticia Acme</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>
                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>
            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>
                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <p style="font-family: serif;">
                            Se ha generado una nueva noticia, por favor ingresa a nuestra app para verla.
                            <br>
                            <br>
                            Gracias!
                            <br>
                            Saludos.
                            <br><br>
                        </p>
                    </div>
                </div>
            </div>
        </div>
        <div>
            <div style="background-color: #FFCC00; height: 7px;"></div>
            <div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
                <div style="margin: auto;text-align:left;">
                    <h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
                </div>
            </div>
        </div>
        <div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
    </div> 
    `, // html body
            });
            res.status(200).json({
                message: 'noticia creada correctamente'
            });
        } else {
            res.status(500).json({
                message: 'No se pudo crear noticia'
            });
        }
    } catch (error) {
        res.status(501).json({
            message: 'Error al crear noticia'
        });
    }
}


const insertImgNews = async (req, res) => {
    const { id_noticia } = req.body;
    const file = req.file.path;
    const verificar = await pool.query(`select * from noticias where id_noticia= $1`, [id_noticia]);
    const url = verificar.rows[0].url;
    //console.log(url);
    if (url === '') {
        const noticia = await pool.query(`update noticias set url=$1 where id_noticia=$2`, [file, id_noticia]);
        res.status(200).json({
            message: 'Imagen cargada correctamente'
        });
    } else {
        try {
            const response = await pool.query('insert into img_noticias (id_noticia,url)values($1,$2)', [id_noticia, file]);
            if (response.rowCount > 0) {
                res.status(200).json({
                    message: 'Imagen cargada correctamente'
                });
            } else {
                res.status(500).json({
                    message: 'No se pudo cargar imagen'
                });
            }
        } catch (error) {
            res.status(501).json({
                message: 'Error al cargar imagen'
            });
        }
    }
}

const getImgNews = async (req, res) => {
    const { id_noticia } = req.body;
    try {
        const response = await pool.query(`select * from img_noticias where id_noticia =$1 order by id_img_not desc`, [id_noticia]);
        res.status(200).json(response.rows);
    } catch (error) {
        res.status(500).json({ message: 'Error al seleccionar imagen de noticia' });
    }
};

const deleteImgNews = async (req, res) => {
    const { id_noticia } = req.body;
    const consulta = await pool.query(`select * from noticias where id_noticia= $1`, [id_noticia]);
    if (consulta.rowCount > 0) {
        const verificar = await pool.query(`select * from img_noticias where id_noticia=$1`, [id_noticia]);
        //console.log(verificar);
        if (verificar.rowCount > 0) {
            try {
                const img = await pool.query(`delete from img_noticias where id_noticia=$1`, [id_noticia]);
                if (img.rowCount > 0) {
                    res.status(200).json({ message: 'Imagen eliminada correctamente' });
                } else {
                    res.status(500).json({ message: 'La noticia no tiene imagenes asociadas ' });
                }
            } catch (error) {
            }
        } else {
            try {
                const response = await pool.query(`update noticias set url='' where id_noticia=$1`, [id_noticia]);
                if (response.rowCount > 0) {
                    res.status(200).json({ message: 'Imagen eliminada correctamente' });
                } else {
                    res.status(500).json({ message: 'La noticia no tiene imagenes asociadas ' });
                }
            } catch (error) {
                res.status(500).json({ message: 'Error al eliminar imagen de noticia' });
            }
        };
    }
    else {
        res.status(500).json({ message: 'No se encuentra la noticia ' });
    }
}


//editar Noticia titulo, subtitulo, cuerpo noticia, fecha_vencimiento
const editNews = async (req, res) => {
    try {
        const { titulo, subtitulo, cuerpo_noticia, fecha_vencimiento, id_categoria, id_noticia } = req.body;
        const consulta = await pool.query('select * from noticias where id_noticia =$1 and id_estado = 1 ', [id_noticia]);
        if (consulta.rowCount > 0) {
            const response = await pool.query('update noticias set titulo = $1, subtitulo = $2, cuerpo_noticia = $3, fecha_vencimiento = $4, id_categoria = $5 where id_noticia = $6', [titulo, subtitulo, cuerpo_noticia, fecha_vencimiento, id_categoria, id_noticia]);
            if (response.rowCount > 0) {
                res.status(200).json({
                    message: 'Noticia editada correctamente'
                });
            } else {
                res.status(500).json({
                    message: 'No pudo editarse noticia'
                });
            }
        } else {
            res.status(500).json({
                message: 'No se puede editar noticia ya que no esta activa'
            });
        }
    }
    catch (error) {
        res.status(500).json({
            message: 'No se pudo editar la noticia server'
        });
    }
}

// Alta User
//0 = Baja
//1 = Alta
const altaNews = async (req, res) => {
    const { id_noticia } = req.body;
    try {
        //Verificar q exista el usuario
        const news = await pool.query('select * from noticias where id_noticia = $1', [id_noticia]);
        if (news.rowCount > 0) {
            const id_estado = 0;
            //Verificar que el estado del usuario sea 0, este de baja
            const verify = await pool.query('select * from noticias where id_noticia = $1 and id_estado=$2', [id_noticia, id_estado]);
            if (verify.rowCount > 0) {
                const id_estado = 1;
                const response = await pool.query('update noticias set id_estado=$1 where id_noticia=$2', [id_estado, id_noticia]);
                res.status(200).json({ message: 'Noticia dada de alta correctamente' });
            } else {
                res.status(501).json({ message: 'La noticia ya está dada de alta' })
            }
        } else {
            res.status(501).json({ message: 'Noticia no encontrada' })
        }
    }
    catch (error) {
        res.status(500).json({ message: 'No se puedo actualizar estado de noticia' });
    }
}


//baja Noticia
const bajaNews = async (req, res) => {
    const { id_noticia } = req.body;
    try {
        //Verificar q exista el usuario
        const news = await pool.query('select * from noticias where id_noticia = $1', [id_noticia]);
        if (news.rowCount > 0) {
            const id_estado = 1;
            //Verificar que el estado de la noticia sea 1, este activa
            const verify = await pool.query('select * from noticias where id_noticia = $1 and id_estado=$2', [id_noticia, id_estado]);
            if (verify.rowCount > 0) {
                const id_estado = 0;
                const response = await pool.query('update noticias set id_estado=$1 where id_noticia=$2', [id_estado, id_noticia]);
                res.status(200).json({ message: 'Noticia dada de baja correctamente' });
            } else {
                res.status(501).json({ message: 'La noticia ya está dada de baja' })
            }
        } else {
            res.status(501).json({ message: 'Noticia no encontrada' })
        }
    }
    catch (error) {
        res.status(500).json({ message: 'No se puedo actualizar estado de noticia' });
    }
}

//obtener todas las categorias
const getCategories = async (req, res) => {
    try {
        const response = await pool.query('select * from categoria_noticias');
        res.status(200).json(response.rows);
    } catch (error) {
        res.status(500).json({ message: 'Error al obtener categorias' });
    }
}


module.exports = {
    getNews,
    getActiveNews,
    getNewsById,
    createNews,
    insertImgNews,
    getImgNews,
    deleteImgNews,
    editNews,
    deleteImgNews,
    altaNews,
    bajaNews,
    getCategories,
} 