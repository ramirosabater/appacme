const { pool } = require('../../../config');
const { transporter } = require('../../../libs/mailer');
const { config } = require('../../../config');




//CAJA CHICA----------------------------------
//Preparar datos para solicitud caja chica
const prepararDatosSolicitudCajaChica = async (id_solicitud_dinero, id_empleado, motivo_solicitud, monto, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);

        const empleado = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1`, [id_empleado]);
        const jefeDirecto = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1 order by e.id_empleado desc`, [empleado.rows[0].jefe_directo]);

        //console.log('Empleado: '+ jefeDirecto.rows[0].nombre, jefeDirecto.rows[0].apellido );
        data.rows[0].datos = data.rows;
        data.rows[0].motivo_solicitud = motivo_solicitud;
        data.rows[0].monto = monto;
        data.rows[0].jefe = jefeDirecto.rows;
        data.rows[0].empleado = empleado.rows;
        //console.log('Mail jefe: ' + jefeDirecto.rows[0].email);

        const html = nuevaSolicitudCajaChica(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to: `"${jefeDirecto.rows[0].email}"`,// Mail a lider
            subject: `Solicitud de dinero ✔`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;

    } catch (error) {
        console.error(error);
    }
}

const nuevaSolicitudCajaChica = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud N°000-${data.id_solicitud_dinero}</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>

                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>${data.empleado[0].apellido}, ${data.empleado[0].nombre} realiza el siguiente tipo de pedido: ${data.descripcion}.
                             </h4>
                        <p style="font-family: serif;">
                            Detalle de solicitud:
                            <br>
                            Numero de solicitud: 000-${data.id_solicitud_dinero}
                            <br>
                            Solicitante: ${data.empleado[0].apellido}, ${data.empleado[0].nombre}	
                            <br>
                            Fecha de solicitud:	${data.fecha_pedido}
                            <br>
                            Monto total solicitado: ${data.monto}
                            <br>
                            Motivo solicitud: ${data.motivo_solicitud}
                                          
                            <br><br>                         
                           
`;
    var htmlParte2 = `      <p>             <br><br>
    Para gestionar las solicitudes de dinero ingresa en:

</p>
</div>
</div>
</div>
</div>

<div>
<div style="background-color: #FFCC00; height: 7px;"></div>
<div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
<div style="margin: auto;text-align:left;">
<h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
</div>
</div>



</div>

<div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
</div>



</body>`

    const html = htmlParte1 + htmlParte2
    return html;
}

//Aprobar pedido Caja chica - jefe directo
const prepararAprobarCajaChicaJefeDirecto = async (id_solicitud_dinero, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);

        const estado = await pool.query(`select es.id_estado_solicitud, es.id_solicitud_dinero,es.id_tipo_estado_solicitud,
        es.motivo, ts.descripcion_tipo_estado, es.fecha, es.id_empleado from estados_solicitud es 
        join tipos_estado_solicitud ts on es.id_tipo_estado_solicitud= ts.id_tipo_estado_solicitud 
        where id_solicitud_dinero=$1 order by id_estado_solicitud desc`, [id_solicitud_dinero]);

        const empleado = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1`, [data.rows[0].id_empleado]);
        const jefeDirecto = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1 order by e.id_empleado desc`, [empleado.rows[0].jefe_directo]);

        //console.log('Empleado: '+ jefeDirecto.rows[0].nombre, jefeDirecto.rows[0].apellido );
        data.rows[0].datos = data.rows;
        data.rows[0].jefe = jefeDirecto.rows;
        data.rows[0].empleado = empleado.rows;
        data.rows[0].estado = estado.rows;
        //console.log('Datos empleado: ' + data.empleado[0].apellido);

        const html = cuerpoAprobarCajaChicaJefeDirecto(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to: `${config.mailAdministracion}`,// Mail a administracion
            subject: `Solicitud de dinero - Aprobado jefe directo✔`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;

    } catch (error) {
        console.error(error);
    }
}

const cuerpoAprobarCajaChicaJefeDirecto = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud N°000-${data.id_solicitud_dinero}</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>

                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>${data.jefe[0].apellido}, ${data.jefe[0].nombre} aprobo el pedido de
                        Caja chica a nombre de
                        <br> ${data.empleado[0].apellido}, ${data.empleado[0].apellido}. 
                        </h4>

                        <p style="font-family: serif;">
                            Estado:
                        </p>  <h4>${data.estado[0].descripcion_tipo_estado}</h4>
                        <p style="font-family: serif;">
                            Detalle de solicitud:
                            <br>
                            Numero de solicitud: 000-${data.id_solicitud_dinero}
                            <br>
                            Solicitante: ${data.empleado[0].apellido}, ${data.empleado[0].apellido}	
                            <br>
                            Fecha de solicitud:	${data.fecha_pedido}
                            <br>
                            Monto total solicitado: ${data.monto_solicitado}
                            <br>
                            Motivo solicitud: ${data.motivo_solicitud}
                                                                      
                                                     
                                                                         
                           
`;
    var htmlParte2 = `      <p>             <br><br>
    Para gestionar las solicitudes de dinero ingresa en:

</p>
</div>
</div>
</div>
</div>

<div>
<div style="background-color: #FFCC00; height: 7px;"></div>
<div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
<div style="margin: auto;text-align:left;">
<h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
</div>
</div>



</div>

<div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
</div>



</body>`

    const html = htmlParte1 + htmlParte2
    return html;
}

//Rechazar pedido Caja chica - jefe directo
const prepararRechazarCajaChicaJefeDirecto = async (id_solicitud_dinero,motivo, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);

        const estado = await pool.query(`select es.id_estado_solicitud, es.id_solicitud_dinero,es.id_tipo_estado_solicitud,
        es.motivo, ts.descripcion_tipo_estado, es.fecha, es.id_empleado from estados_solicitud es 
        join tipos_estado_solicitud ts on es.id_tipo_estado_solicitud= ts.id_tipo_estado_solicitud 
        where id_solicitud_dinero=$1 order by id_estado_solicitud desc`, [id_solicitud_dinero]);

        const empleado = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1`, [data.rows[0].id_empleado]);
        const jefeDirecto = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1 order by e.id_empleado desc`, [empleado.rows[0].jefe_directo]);

        //console.log('Empleado: '+ jefeDirecto.rows[0].nombre, jefeDirecto.rows[0].apellido );
        data.rows[0].datos = data.rows;
        data.rows[0].jefe = jefeDirecto.rows;
        data.rows[0].empleado = empleado.rows;
        data.rows[0].estado = estado.rows;
        data.rows[0].motivo = motivo;
        //console.log('Mail empleado: ' + empleado.rows[0].email);

        const html = cuerpoRechazarCajaChicaJefeDirecto(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to: `"${empleado.rows[0].email}"`,// Mail a empleado
            subject: `Solicitud de dinero - Rechazado jefe directo`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;

    } catch (error) {
        console.error(error);
    }
}

const cuerpoRechazarCajaChicaJefeDirecto = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud 000-${data.id_solicitud_dinero} rechazada</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>

                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>${data.jefe[0].apellido}, ${data.jefe[0].nombre} rechazó el pedido ${data.datos[0].descripcion}.
                             </h4>

                        <p style="font-family: serif;">
                            Estado:
                        </p>  <h4>${data.estado[0].descripcion_tipo_estado}</h4>
                        Motivo:
                        </p> <p>${data.motivo}</p>
                        
                        <br>
                        <p style="font-family: serif;">
                            Detalle de solicitud:
                            <br>
                            <br>
                            Numero de solicitud: 000-${data.id_solicitud_dinero}
                            <br>
                            Solicitante: ${data.empleado[0].apellido}, ${data.empleado[0].apellido}	
                            <br>
                            Fecha de solicitud:	${data.fecha_pedido}
                            <br>
                            Monto total solicitado: ${data.monto_solicitado}
                            <br>
                            Motivo solicitud: ${data.motivo_solicitud}
                                                                      
                                                         
                                             
                           
`;
    var htmlParte2 = `      <p>             <br><br>
    Para gestionar las solicitudes de dinero ingresa en:

</p>
</div>
</div>
</div>
</div>

<div>
<div style="background-color: #FFCC00; height: 7px;"></div>
<div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
<div style="margin: auto;text-align:left;">
<h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
</div>
</div>



</div>

<div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
</div>



</body>`

    const html = htmlParte1 + htmlParte2
    return html;
}


//Aprobar pedido Caja chica - administracion
const prepararAprobarCajaChicaAdministracion = async (id_solicitud_dinero, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);

        const estado = await pool.query(`select es.id_estado_solicitud, es.id_solicitud_dinero,es.id_tipo_estado_solicitud,
        es.motivo, ts.descripcion_tipo_estado, es.fecha, es.id_empleado from estados_solicitud es 
        join tipos_estado_solicitud ts on es.id_tipo_estado_solicitud= ts.id_tipo_estado_solicitud 
        where id_solicitud_dinero=$1 order by id_estado_solicitud desc`, [id_solicitud_dinero]);

        const empleado = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1`, [data.rows[0].id_empleado]);
        const jefeDirecto = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1 order by e.id_empleado desc`, [empleado.rows[0].jefe_directo]);

        //console.log('Empleado: '+ jefeDirecto.rows[0].nombre, jefeDirecto.rows[0].apellido );
        data.rows[0].datos = data.rows;
        data.rows[0].jefe = jefeDirecto.rows;
        data.rows[0].empleado = empleado.rows;
        data.rows[0].estado = estado.rows;
        //console.log('Datos empleado: ' + data.empleado[0].apellido);

        const html = cuerpoAprobarCajaChicaAdministracion(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to: `"${empleado.rows[0].email}"`,// Mail a empleado
            subject: `Solicitud de dinero - Aprobado administración✔`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;

    } catch (error) {
        console.error(error);
    }
}

const cuerpoAprobarCajaChicaAdministracion = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud N°000-${data.id_solicitud_dinero}</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>

                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>Su pedido N°${data.id_solicitud_dinero} del tipo: ${data.descripcion}
                           , ha sido aprobado.
                             </h4>

                        <p style="font-family: serif;">
                            Se le notifica que debe pasar por administracion a retirar el dinero.
                        </p>                          
                                                      
                           
`;
    var htmlParte2 = `      <p>             <br><br>
    Para gestionar las solicitudes de dinero ingresa en:

</p>
</div>
</div>
</div>
</div>

<div>
<div style="background-color: #FFCC00; height: 7px;"></div>
<div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
<div style="margin: auto;text-align:left;">
<h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
</div>
</div>



</div>

<div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
</div>



</body>`

    const html = htmlParte1 + htmlParte2
    return html;
}


//Rechazar pedido Caja chica - administracion
const prepararRechazarCajaChicaAdministracion = async (id_solicitud_dinero,motivo, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);

        const estado = await pool.query(`select es.id_estado_solicitud, es.id_solicitud_dinero,es.id_tipo_estado_solicitud,
        es.motivo, ts.descripcion_tipo_estado, es.fecha, es.id_empleado from estados_solicitud es 
        join tipos_estado_solicitud ts on es.id_tipo_estado_solicitud= ts.id_tipo_estado_solicitud 
        where id_solicitud_dinero=$1 order by id_estado_solicitud desc`, [id_solicitud_dinero]);

        const empleado = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1`, [data.rows[0].id_empleado]);
        const jefeDirecto = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1 order by e.id_empleado desc`, [empleado.rows[0].jefe_directo]);

        //console.log('Empleado: '+ jefeDirecto.rows[0].nombre, jefeDirecto.rows[0].apellido );
        data.rows[0].datos = data.rows;
        data.rows[0].jefe = jefeDirecto.rows;
        data.rows[0].empleado = empleado.rows;
        data.rows[0].estado = estado.rows;
        data.rows[0].motivo = motivo;
        //console.log('Datos empleado: ' + data.empleado[0].apellido);

        const html = cuerpoRechazarCajaChicaAdministracion(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to: `"${empleado.rows[0].email}"`,// Mail a empleado
            subject: `Solicitud de dinero - Rechazado administracion`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;

    } catch (error) {
        console.error(error);
    }
}

const cuerpoRechazarCajaChicaAdministracion = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud 000-${data.id_solicitud_dinero} rechazada</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>

                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>Administración rechazó el pedido ${data.datos[0].descripcion}.
                             </h4>

                        <p style="font-family: serif;">
                            Estado:
                        </p>  <h4>${data.estado[0].descripcion_tipo_estado}</h4>
                        <p style="font-family: serif;">Motivo:</p>
                        <p>${data.motivo}</p>
                        
                    
                        <p style="font-family: serif;">
                            Detalle de solicitud:
                            <br>
                            Numero de solicitud: 000-${data.id_solicitud_dinero}
                            <br>
                            Solicitante: ${data.empleado[0].apellido}, ${data.empleado[0].apellido}	
                            <br>
                            Fecha de solicitud:	${data.fecha_pedido}
                            <br>
                            Monto total solicitado: ${data.monto_solicitado}
                            <br>
                            Motivo solicitud: ${data.motivo_solicitud}
                                                                      
                                                         
                                             
                           
`;
    var htmlParte2 = `      <p>             <br><br>
    Para gestionar las solicitudes de dinero ingresa en:

</p>
</div>
</div>
</div>
</div>

<div>
<div style="background-color: #FFCC00; height: 7px;"></div>
<div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
<div style="margin: auto;text-align:left;">
<h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
</div>
</div>



</div>

<div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
</div>



</body>`

    const html = htmlParte1 + htmlParte2
    return html;
}

//Preparar datos para rendicion caja chica
const prepararDatosRendicionCajaChica = async (id_solicitud_dinero, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);

        const estado = await pool.query(`select es.id_estado_solicitud, es.id_solicitud_dinero,es.id_tipo_estado_solicitud,
        es.motivo, ts.descripcion_tipo_estado, es.fecha, es.id_empleado from estados_solicitud es 
        join tipos_estado_solicitud ts on es.id_tipo_estado_solicitud= ts.id_tipo_estado_solicitud 
        where id_solicitud_dinero=$1 order by id_estado_solicitud desc`, [id_solicitud_dinero]);

        const empleado = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1`, [data.rows[0].id_empleado]);
        const jefeDirecto = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1 order by e.id_empleado desc`, [empleado.rows[0].jefe_directo]);

        //console.log('Empleado: '+ jefeDirecto.rows[0].nombre, jefeDirecto.rows[0].apellido );
        data.rows[0].datos = data.rows;
        data.rows[0].jefe = jefeDirecto.rows;
        data.rows[0].empleado = empleado.rows;
        data.rows[0].estado = estado.rows;
        //console.log('Datos empleado: ' + data.empleado[0].apellido);

        const html = nuevaRendicionCajaChica(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to:  `"${jefeDirecto.rows[0].email}"`,// Mail a lider
            subject: `Solicitud de dinero - Presentación de rendición`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;

    } catch (error) {
        console.error(error);
    }
}


const nuevaRendicionCajaChica = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud N°000-${data.id_solicitud_dinero} - ${data.descripcion}</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>

                <div style="text-align: left;">
                <p style="font-family: serif;">Hola</p>
                ${data.empleado[0].apellido}, ${data.empleado[0].nombre} ha realizado la siguiente acción:
                
           
                     <h4>${data.estado[0].descripcion_tipo_estado}</h4>
                        <p style="font-family: serif;">
                            Detalle de solicitud:
                            <br>
                            Tipo de solicitud: ${data.descripcion}
                            <br>
                            Numero de solicitud: 000-${data.id_solicitud_dinero}
                            <br>
                            Solicitante: ${data.empleado[0].apellido}, ${data.empleado[0].nombre}	
                            <br>
                            Fecha de solicitud:	${data.fecha_pedido}
                            <br>
                            Monto total solicitado: ${data.datos[0].monto_solicitado}
                            <br>
                            Motivo solicitud: ${data.motivo_solicitud}
                                          
                            <br><br>                         
                           
`;
    var htmlParte2 = `      <p>             <br><br>
    Para gestionar las solicitudes de dinero ingresa en:

</p>
</div>
</div>
</div>
</div>

<div>
<div style="background-color: #FFCC00; height: 7px;"></div>
<div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
<div style="margin: auto;text-align:left;">
<h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
</div>
</div>



</div>

<div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
</div>



</body>`

    const html = htmlParte1 + htmlParte2
    return html;
}

//Preparar datos para rendicion caja chica
const prepararDatosRendicionTarjeta = async (id_solicitud_dinero, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);

        const estado = await pool.query(`select es.id_estado_solicitud, es.id_solicitud_dinero,es.id_tipo_estado_solicitud,
        es.motivo, ts.descripcion_tipo_estado, es.fecha, es.id_empleado from estados_solicitud es 
        join tipos_estado_solicitud ts on es.id_tipo_estado_solicitud= ts.id_tipo_estado_solicitud 
        where id_solicitud_dinero=$1 order by id_estado_solicitud desc`, [id_solicitud_dinero]);

        const empleado = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1`, [data.rows[0].id_empleado]);
        const jefeDirecto = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1 order by e.id_empleado desc`, [empleado.rows[0].jefe_directo]);

        //console.log('Empleado: '+ jefeDirecto.rows[0].nombre, jefeDirecto.rows[0].apellido );
        data.rows[0].datos = data.rows;
        data.rows[0].jefe = jefeDirecto.rows;
        data.rows[0].empleado = empleado.rows;
        data.rows[0].estado = estado.rows;
        //console.log('Datos empleado: ' + data.empleado[0].apellido);

        const html = nuevaRendicionTarjeta(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to:  `${config.mailTesoreria}`,// Mail a Tesorería
            subject: `Solicitud de dinero - Presentación de rendición`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;

    } catch (error) {
        console.error(error);
    }
}


const nuevaRendicionTarjeta = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud N°000-${data.id_solicitud_dinero} - ${data.descripcion}</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>

                <div style="text-align: left;">
                <p style="font-family: serif;">Hola</p>
                ${data.empleado[0].apellido}, ${data.empleado[0].nombre} ha realizado la siguiente acción:
                
           
                     <h4>${data.estado[0].descripcion_tipo_estado}</h4>
                        <p style="font-family: serif;">
                            Detalle de solicitud:
                            <br>
                            Tipo de solicitud: ${data.descripcion}
                            <br>
                            Numero de solicitud: 000-${data.id_solicitud_dinero}
                            <br>
                            Solicitante: ${data.empleado[0].apellido}, ${data.empleado[0].nombre}	
                            <br>
                            Fecha de solicitud:	${data.fecha_pedido}
                            <br>
                            Monto total solicitado: ${data.datos[0].monto_solicitado}
                            <br>
                            Motivo solicitud: ${data.motivo_solicitud}
                                          
                            <br><br>                         
                           
`;
    var htmlParte2 = `      <p>             <br><br>
    Para gestionar las solicitudes de dinero ingresa en:

</p>
</div>
</div>
</div>
</div>

<div>
<div style="background-color: #FFCC00; height: 7px;"></div>
<div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
<div style="margin: auto;text-align:left;">
<h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
</div>
</div>



</div>

<div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
</div>



</body>`

    const html = htmlParte1 + htmlParte2
    return html;
}

//Aprobar rendicion Caja chica - administracion
const prepararAprobarRendicionCajaChicaJefeDirecto = async (id_solicitud_dinero, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);

        const estado = await pool.query(`select es.id_estado_solicitud, es.id_solicitud_dinero,es.id_tipo_estado_solicitud,
        es.motivo, ts.descripcion_tipo_estado, es.fecha, es.id_empleado from estados_solicitud es 
        join tipos_estado_solicitud ts on es.id_tipo_estado_solicitud= ts.id_tipo_estado_solicitud 
        where id_solicitud_dinero=$1 order by id_estado_solicitud desc`, [id_solicitud_dinero]);

        const empleado = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1`, [data.rows[0].id_empleado]);
        const jefeDirecto = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1 order by e.id_empleado desc`, [empleado.rows[0].jefe_directo]);

        //console.log('Empleado: '+ jefeDirecto.rows[0].nombre, jefeDirecto.rows[0].apellido );
        data.rows[0].datos = data.rows;
        data.rows[0].jefe = jefeDirecto.rows;
        data.rows[0].empleado = empleado.rows;
        data.rows[0].estado = estado.rows;
        //console.log('Datos empleado: ' + data.empleado[0].apellido);

        const html = cuerpoAprobarRendicionCajaChicaJefeDirecto(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to:  `${config.mailAdministracion}`,// Mail a administracion
            subject: `Solicitud de dinero - Aprobada rendición jefe directo✔`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;

    } catch (error) {
        console.error(error);
    }
}

const cuerpoAprobarRendicionCajaChicaJefeDirecto = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud 000-${data.id_solicitud_dinero} con rendicion aprobada</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>

                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>${data.jefe[0].apellido}, ${data.jefe[0].nombre} aprobo la rendición sobre el pedido N° ${data.datos[0].descripcion}.
                             </h4>

                        <p style="font-family: serif;">
                            Estado:
                        </p>  <h4>${data.estado[0].descripcion_tipo_estado}</h4>
                                                
                    
                        <p style="font-family: serif;">
                            Detalle de solicitud:
                            <br>
                            Numero de solicitud: 000-${data.id_solicitud_dinero}
                            <br>
                            Solicitante: ${data.empleado[0].apellido}, ${data.empleado[0].apellido}	
                            <br>
                            Fecha de solicitud:	${data.fecha_pedido}
                            <br>
                            Monto total solicitado: ${data.monto_solicitado}
                            <br>
                            Motivo solicitud: ${data.motivo_solicitud}
                                                                      
                                                         
                                             
                           
`;
    var htmlParte2 = `      <p>             <br><br>
    Para gestionar las solicitudes de dinero ingresa en:

</p>
</div>
</div>
</div>
</div>

<div>
<div style="background-color: #FFCC00; height: 7px;"></div>
<div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
<div style="margin: auto;text-align:left;">
<h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
</div>
</div>



</div>

<div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
</div>



</body>`

    const html = htmlParte1 + htmlParte2
    return html;
}


//Rechazar rendicion Caja chica - jefe directo
const prepararRechazarRendicionCajaChicaJefeDirecto = async (id_solicitud_dinero,motivo, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);

        const estado = await pool.query(`select es.id_estado_solicitud, es.id_solicitud_dinero,es.id_tipo_estado_solicitud,
        es.motivo, ts.descripcion_tipo_estado, es.fecha, es.id_empleado from estados_solicitud es 
        join tipos_estado_solicitud ts on es.id_tipo_estado_solicitud= ts.id_tipo_estado_solicitud 
        where id_solicitud_dinero=$1 order by id_estado_solicitud desc`, [id_solicitud_dinero]);

        const empleado = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1`, [data.rows[0].id_empleado]);
        const jefeDirecto = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1 order by e.id_empleado desc`, [empleado.rows[0].jefe_directo]);

        //console.log('Empleado: '+ jefeDirecto.rows[0].nombre, jefeDirecto.rows[0].apellido );
        data.rows[0].datos = data.rows;
        data.rows[0].jefe = jefeDirecto.rows;
        data.rows[0].empleado = empleado.rows;
        data.rows[0].estado = estado.rows;
        data.rows[0].motivo = motivo;
        //console.log('Datos empleado: ' + data.empleado[0].apellido);

        const html = cuerpoRechazarRendicionCajaChicaJefeDirecto(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to: `"${empleado.rows[0].email}"`,// Mail a empleado
            subject: `Solicitud de dinero - Rendición rechazada`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;

    } catch (error) {
        console.error(error);
    }
}

const cuerpoRechazarRendicionCajaChicaJefeDirecto = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud 000-${data.id_solicitud_dinero} - Rendición rechaza</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>

                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>${data.jefe[0].apellido}, ${data.jefe[0].nombre} rechazo la rendición sobre el pedido N° ${data.id_solicitud_dinero}.
                        <br> Comuniquese con su lider antes de presentar una nueva rendición.
                             </h4>

                        <p style="font-family: serif;">
                            Estado:
                        </p>  <h4>${data.estado[0].descripcion_tipo_estado}</h4>
                                                
                    
                        <p style="font-family: serif;">
                            Detalle de solicitud:
                            <br>
                            Numero de solicitud: 000-${data.id_solicitud_dinero}
                            <br>
                            Solicitante: ${data.empleado[0].apellido}, ${data.empleado[0].apellido}	
                            <br>
                            Fecha de solicitud:	${data.fecha_pedido}
                            <br>
                            Monto total solicitado: ${data.monto_solicitado}
                            <br>
                            Motivo solicitud: ${data.motivo_solicitud}
                                                                      
                                                         
                                             
                           
`;
    var htmlParte2 = `      <p>             <br><br>
    Para gestionar las solicitudes de dinero ingresa en:

</p>
</div>
</div>
</div>
</div>

<div>
<div style="background-color: #FFCC00; height: 7px;"></div>
<div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
<div style="margin: auto;text-align:left;">
<h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
</div>
</div>



</div>

<div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
</div>



</body>`

    const html = htmlParte1 + htmlParte2
    return html;
}


//Aprobar rendicion Caja chica - administracion
const prepararAprobarRendicionCajaChicaAdministracion = async (id_solicitud_dinero, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);

        const estado = await pool.query(`select es.id_estado_solicitud, es.id_solicitud_dinero,es.id_tipo_estado_solicitud,
        es.motivo, ts.descripcion_tipo_estado, es.fecha, es.id_empleado from estados_solicitud es 
        join tipos_estado_solicitud ts on es.id_tipo_estado_solicitud= ts.id_tipo_estado_solicitud 
        where id_solicitud_dinero=$1 order by id_estado_solicitud desc`, [id_solicitud_dinero]);

        const empleado = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1`, [data.rows[0].id_empleado]);
        const jefeDirecto = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1 order by e.id_empleado desc`, [empleado.rows[0].jefe_directo]);

        //console.log('Empleado: '+ jefeDirecto.rows[0].nombre, jefeDirecto.rows[0].apellido );
        data.rows[0].datos = data.rows;
        data.rows[0].jefe = jefeDirecto.rows;
        data.rows[0].empleado = empleado.rows;
        data.rows[0].estado = estado.rows;
        //console.log('Datos empleado: ' + data.empleado[0].apellido);

        const html = cuerpoAprobarRendicionCajaChicaAdministracion(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to: `"${empleado.rows[0].email}"`,// Mail a empleado
            subject: `Solicitud de dinero - Aprobada rendición administración✔`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;

    } catch (error) {
        console.error(error);
    }
}

const cuerpoAprobarRendicionCajaChicaAdministracion = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud N°000-${data.id_solicitud_dinero} con rendicion aprobada</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>

                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>Administración aprobó la rendición sobre el pedido N° 000-${data.id_solicitud_dinero}.
                        El circuito a sido cerrado por lo que no se pueden realizar mas acciones.
                             </h4>

                        <p style="font-family: serif;">
                            Estado:
                        </p>  <h4>${data.estado[0].descripcion_tipo_estado}</h4>
                                                
                    
                        <p style="font-family: serif;">
                            Detalle de solicitud:
                            <br>
                            Numero de solicitud: 000-${data.id_solicitud_dinero}
                            <br>
                            Solicitante: ${data.empleado[0].apellido}, ${data.empleado[0].apellido}	
                            <br>
                            Fecha de solicitud:	${data.fecha_pedido}
                            <br>
                            Monto total solicitado: ${data.monto_solicitado}
                            <br>
                            Motivo solicitud: ${data.motivo_solicitud}
                                                                      
                                                         
                                             
                           
`;
    var htmlParte2 = `      <p>             <br><br>
    Para gestionar las solicitudes de dinero ingresa en:

</p>
</div>
</div>
</div>
</div>

<div>
<div style="background-color: #FFCC00; height: 7px;"></div>
<div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
<div style="margin: auto;text-align:left;">
<h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
</div>
</div>



</div>

<div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
</div>



</body>`

    const html = htmlParte1 + htmlParte2
    return html;
}




//Rechazar rendicion Caja chica - jefe directo
const prepararRechazarRendicionCajaChicaAdministracion = async (id_solicitud_dinero,motivo, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);

        const estado = await pool.query(`select es.id_estado_solicitud, es.id_solicitud_dinero,es.id_tipo_estado_solicitud,
        es.motivo, ts.descripcion_tipo_estado, es.fecha, es.id_empleado from estados_solicitud es 
        join tipos_estado_solicitud ts on es.id_tipo_estado_solicitud= ts.id_tipo_estado_solicitud 
        where id_solicitud_dinero=$1 order by id_estado_solicitud desc`, [id_solicitud_dinero]);

        const empleado = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1`, [data.rows[0].id_empleado]);
        const jefeDirecto = await pool.query(`select e.id_empleado ,u.nombre, u.apellido, e.jefe_directo, e.n_legajo,u.email from empleados e,usuarios u where e.id_empleado = u.id_usuario and e.id_empleado = $1 order by e.id_empleado desc`, [empleado.rows[0].jefe_directo]);

        //console.log('Empleado: '+ jefeDirecto.rows[0].nombre, jefeDirecto.rows[0].apellido );
        data.rows[0].datos = data.rows;
        data.rows[0].jefe = jefeDirecto.rows;
        data.rows[0].empleado = empleado.rows;
        data.rows[0].estado = estado.rows;
        data.rows[0].motivo = motivo;
        //console.log('Datos empleado: ' + data.empleado[0].apellido);

        const html = cuerpoRechazarRendicionCajaChicaAdministracion(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to: `"${empleado.rows[0].email}"`,// Mail a empleado. posiblemente a lider??
            subject: `Solicitud de dinero - Rendición rechazada`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;

    } catch (error) {
        console.error(error);
    }
}

const cuerpoRechazarRendicionCajaChicaAdministracion = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            style="font-family: 'Franklin Gothic Medium', 'Arial Narrow', Arial, sans-serif; text-align: center; margin: auto;">
            <div>
                <div style="background-color: #333333; height: 22px;">

                </div>
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud 000-${data.id_solicitud_dinero} - Rendición rechaza</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
                            alt="Logo" style="height: 75px;padding-top: 2.5em;"></a>

                </div>
                <div style="background-color: #333333;height: 10px;"></div>
            </div>


            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>

                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>Administración rechazó la rendición sobre el pedido N° 000-${data.id_solicitud_dinero}.
                        <br> Comuniquese con su lider antes de presentar una nueva rendición.
                             </h4>

                        <p style="font-family: serif;">
                            Estado:
                        </p>  <h4>${data.estado[0].descripcion_tipo_estado}</h4>
                                                
                    
                        <p style="font-family: serif;">
                            Detalle de solicitud:
                            <br>
                            Numero de solicitud: 000-${data.id_solicitud_dinero}
                            <br>
                            Solicitante: ${data.empleado[0].apellido}, ${data.empleado[0].apellido}	
                            <br>
                            Fecha de solicitud:	${data.fecha_pedido}
                            <br>
                            Monto total solicitado: ${data.monto_solicitado}
                            <br>
                            Motivo solicitud: ${data.motivo_solicitud}
                                                                      
                                                         
                                             
                           
`;
    var htmlParte2 = `      <p>             <br><br>
    Para gestionar las solicitudes de dinero ingresa en:

</p>
</div>
</div>
</div>
</div>

<div>
<div style="background-color: #FFCC00; height: 7px;"></div>
<div style="background-color: #E8E8E8;height: 65px; padding-top: 1px;">
<div style="margin: auto;text-align:left;">
<h2><a href="http://app.acme.com.ar/" style="margin-left: 10%;">app.acme.com.ar</a></h2>
</div>
</div>



</div>

<div style="background-color: #333333;height: 10px;margin-bottom: 4em;"></div>
</div>



</body>`

    const html = htmlParte1 + htmlParte2
    return html;
}




module.exports = {
    prepararDatosSolicitudCajaChica,
    prepararAprobarCajaChicaJefeDirecto,
    prepararRechazarCajaChicaJefeDirecto,
    prepararAprobarCajaChicaAdministracion,
    prepararRechazarCajaChicaAdministracion,
    prepararDatosRendicionCajaChica,
    prepararAprobarRendicionCajaChicaJefeDirecto,
    prepararRechazarRendicionCajaChicaJefeDirecto,
    prepararAprobarRendicionCajaChicaAdministracion,
    prepararRechazarRendicionCajaChicaAdministracion,
    prepararDatosRendicionTarjeta
}