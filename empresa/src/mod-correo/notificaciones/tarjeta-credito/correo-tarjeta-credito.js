...
//Tarjeta Credito----------------------------------
//Preparar datos para solicitud tarjetaCredio
const prepararDatosSolicitudTarjetaCredito = async (id_solicitud_dinero, id_empleado, motivo_solicitud, monto, req, res) => {
    try {
        ...
        const html = nuevaSolicitudTarjetaCredito(data.rows[0]);
        ...
        await transporter.sendMail({
            ...
            to:  `${config.mailTesoreria}`/* `tesoreria@acme.com.ar` */,
...
const nuevaSolicitudTarjetaCredito = (data) => {
    const htmlParte1 = `
    <body>
...
const prepararAprobarRendicionTarjetaCreditoTesoreria = async (id_solicitud_dinero, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);
        ...
        const html = cuerpoAprobarRendicionTarjetaCreditoTesoreria(data.rows[0]);
        //console.log('Mail Tesorería:', config.mailRRHH);
...
const cuerpoAprobarRendicionTarjetaCreditoTesoreria = (data) => {
    const htmlParte1 = `
<body>
    <div style="margin: auto;width: 75%;">
        <div
...
const prepararRechazarRendicionTarjetaCreditoTesoreria = async (id_solicitud_dinero,motivo, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);
        ...
        const html = cuerpoRechazarRendicionTarjetaCreditoTesoreria(data.rows[0]);
...
const cuerpoRechazarRendicionTarjetaCreditoTesoreria = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            ...
            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>
                    ...
                    <div style="text-align: left;">
                        ...
                        <br> Comuniquese con el area antes de presentar una nueva rendición.
                             </h4>
...
module.exports = {
    prepararDatosSolicitudTarjetaCredito,
    prepararRechazarRendicionTarjetaCreditoTesoreria,
    prepararAprobarRendicionTarjetaCreditoTesoreria
}