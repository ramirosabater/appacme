...
//Aprobar pedido viaticos deslocalizados - jefe directo
const preparaSolicitudViaticosDeslocalizados = async (id_solicitud_dinero, req, res) => {
    try {
        ...
        const html = cuerpoPrepararSolicitudViaticosDeslocalizados(data.rows[0]);
        ...
        await transporter.sendMail({
            ...
            to: `${config.mailRRHH}`,// Mail a RRHH
            subject: `Solicitud de dinero - Viaticos Deslocalizados✔`,
            html: html,
        });
...
const cuerpoPrepararSolicitudViaticosDeslocalizados = (data) => {
    const htmlParte1 = `
    <body>
            ...
            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>
                    ...
                    <div style="text-align: left;">
                        ...
                        <h4>${data.empleado[0].apellido}, ${data.empleado[0].apellido} realizo el siguiente pedido del tipo: 
                        ${data.descripcion}.
                        ...
                        <p style="font-family: serif;">
                            ...
                            <br>
                            Cantidad de días: ${data.cantidad_dias}
                            <br>
                            Motivo de Solicitud: ${data.motivo_solicitud}
...
//Aprobar pedido viaticos deslocalizados - jefe directo
const prepararAprobarViaticosDeslocalizadosRRHH = async (id_solicitud_dinero, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);
        ...
        const html = cuerpoAprobarViaticosDeslocalizadosRRHH(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to: `${config.mailTesoreria}`,// Mail a Tesoreria y Empleado ??
            subject: `Solicitud de dinero - Aprobado por Recursos Humanos✔`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;
...
const cuerpoAprobarViaticosDeslocalizadosRRHH = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            ...
            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>
                    ...
                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>Recursos Humanos aprobo el pedido de ${data.descripcion} a nombre de 
                        <br> ${data.empleado[0].apellido}, ${data.empleado[0].apellido}.
                        </h4>
                        <p style="font-family: serif;">En un plazo de 48hs habiles estará preparado el dinero para pasar a retirar por Tesorería.</p>
...
const prepararRechazarViaticosDeslocalizadosRRHH = async (id_solicitud_dinero, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);
        ...
        const html = cuerpoRechazarViaticosDeslocalizadosRRHH(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to:`"${empleado.rows[0].email}"`,// Mail a empleado
            subject: `Solicitud de dinero - Rechazada por Recursos Humanos`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;
...
const cuerpoRechazarViaticosDeslocalizadosRRHH = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            ...
            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>
                    ...
                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>Recursos Humanos rechazo el pedido de ${data.descripcion} a nombre de 
                        <br> ${data.empleado[0].apellido}, ${data.empleado[0].apellido}.
                        </h4>
...
//Aprobar pedido Caja chica - jefe directo
const prepararFinalizarCircuitoViaticosDeslocalizados = async (id_solicitud_dinero, req, res) => {
    try {
        const data = await pool.query(`select *, to_char(s.fecha_solicitud::timestamp with time zone, 'dd/MM/yyyy'::text) AS "fecha_pedido" from solicitud_dinero s join tipos_solicitudes ts 
        on s.id_tipo_solicitud=ts.id_tipo_solicitud where id_solicitud_dinero=$1`, [id_solicitud_dinero]);
        ...
        const html = cuerpoFinalizarCircuitoViaticosDeslocalizados(data.rows[0]);

        await transporter.sendMail({
            from: `"Solicitud de dinero N° 000-${data.rows[0].id_solicitud_dinero}" <notificaciones@acme.com.ar>`,//cambiar mail por notificaciones,
            to: `"${empleado.rows[0].email}"`,// Mail a empleado
            subject: `Solicitud de dinero - Aprobado Tesorería✔`,
            html: html,
        });
        //res.status(200).json('Operación realizada con éxito');;
...
const cuerpoFinalizarCircuitoViaticosDeslocalizados = (data) => {
    const htmlParte1 = `
    <body>
    <div style="margin: auto;width: 75%;">
        <div
            ...
            <div>
                ...
                <div style="background-color: #FFFFFF; height:25px; ">
                    <h4 style="margin-top: 5px;">Solicitud N°000-${data.id_solicitud_dinero} - Cierre de circuito</h4>
                </div>
                <div style="background-color: #FFCC00 ;height:180px;">
                    <a href="https://ibb.co/ZBbvhLt"><img
                            src="data:image/png;base64,"
            ...
            <div style="padding: 2em 0 2em;margin: 0 10% 0 10% ">
                <div>
                    ...
                    <div style="text-align: left;">
                        <p style="font-family: serif;">Hola</p>
                        <h4>Tesorería aprobo el pedido de ${data.descripcion} a nombre de 
                        <br> ${data.empleado[0].apellido}, ${data.empleado[0].apellido}.
                        </h4>
                        <p style="font-family: serif;">El circuito fue cerrado por lo que no se puede realizar ninguna acción sobre el pedido.</p>
...
module.exports = {
    preparaSolicitudViaticosDeslocalizados,
    prepararAprobarViaticosDeslocalizadosRRHH,
    prepararRechazarViaticosDeslocalizadosRRHH,
    prepararFinalizarCircuitoViaticosDeslocalizados      
}
